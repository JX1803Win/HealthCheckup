...
<body>
	<div class="container">
		...
		<table class="table table-bordered">
			...
			<tbody>
				<c:forEach items="${rbList}" var="menu" varStatus="vs">
					<tr>
						<td>${(pageNo-1)*3+vs.index+1}</td>
						<c:if test="${menu.preMenu==0}">
							<td style="color: gray">${menu.menuName}</td>
							<td class="text-center">
								<button class="btn btn-primary x-right" data-toggle="modal"
									data-target="#myModal2"
									onclick="alter('${menu.permissionsId}','${menu.menuName}')">修改</button>
								<a href="<%=path%>role/del.action?permissionsId=${menu.permissionsId}"
								onclick="return del()">
