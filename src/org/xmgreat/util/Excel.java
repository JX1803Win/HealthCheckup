     ...
     */
	   ...
	   */
		public  List<PhyCardBean> getExcelInfo(MultipartFile mFile) {
			...
			System.out.println(fileName+"Excel");
			try {
	   ...
	   */
		private List<PhyCardBean> readExcelValue(Workbook wb) {
			...
			List<PhyCardBean> phyCardList = new ArrayList<PhyCardBean>();
			// 循环Excel行数
			for (int r = 1; r < totalRows; r++) {
				...
				PhyCardBean phyCardBean = new PhyCardBean();
				// 循环Excel的列
					...
					if (null != cell) {
						...
						if (c == 0) {
							if(cell.getStringCellValue()!=null) {
								...
								phyCardBean.setPhyCardId(id);
							}
						} else if (c == 1) {
							if(cell.getStringCellValue()!=null) {
								Integer state= Integer.valueOf(cell.getStringCellValue());
								phyCardBean.setParameterId(state);
							}
						} 
					}
				}
				// 添加到list
				phyCardList.add(phyCardBean);
			}
			return phyCardList;
		}
